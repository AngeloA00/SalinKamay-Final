...
X_PATH = os.path.join(BASE_DIR, "dataset", "processed", "X_custom_fsl.npy")
Y_PATH = os.path.join(BASE_DIR, "dataset", "processed", "y_custom_fsl.npy")
CLASSES_PATH = os.path.join(BASE_DIR, "dataset", "metadata", "custom_fsl_classes.json")
MODEL_OUT = os.path.join(BASE_DIR, "app", "models", "custom_fsl.keras")
...
def main():
    if not (os.path.exists(X_PATH) and os.path.exists(Y_PATH)):
        print("Error: Processed custom_fsl data not found. Run extract_custom_fsl.py first.")
        return

    print("Loading extracted custom_fsl dataset...")
    X = np.load(X_PATH)
    y = np.load(Y_PATH)
    ...
    print("\nStarting Training for custom_fsl model...")
    model.fit(
        X_train, y_train,
